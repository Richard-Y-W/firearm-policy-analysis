...
DATA = ROOT / "data" / "processed" / "analysis_panel_total_firearm_deaths_1999_2024.csv"
...
def make_event_plot(df, adopters, controls):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(OUT_FIGS / "event_time_total_firearm_deaths.png", dpi=300)
    plt.close()
...
def main():
    ...
    results_df.to_csv(OUT_TABLES / "welch_results_total_firearm_deaths.csv", index=False)

    if state_scores is not None:
        state_scores.to_csv(OUT_TABLES / "state_level_A_3y_total_firearm_deaths.csv", index=False)

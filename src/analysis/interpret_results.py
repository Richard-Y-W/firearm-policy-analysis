...
def load_tables():
    ...
    return did, welch, hetero, state_level
...
def build_main_conclusion(did, welch):
    ...
    if fs["p_post_permitless"] < 0.05 and nfs["p_post_permitless"] < 0.05 and ts["p_post_permitless"] < 0.05:
        ...
        lines.append("The mechanism evidence is mixed and should be described cautiously.")
...
def build_heterogeneity_section(hetero):
    ...
    if sig.empty:
        lines.extend(["No heterogeneity interaction survives the 5% level in this first-pass specification.", ""])
        return "\n".join(lines)

    for outcome_label, g in sig.groupby("outcome_label"):
        best = g.sort_values("interaction_p").iloc[0]
        lines.append(
...
def build_political_selection_section(state_level):
    lines = [
        "## Political Selection",
        "",
    ...
    if len(adopter) > 0 and len(non) > 0:
        lines.extend(
            [
                ...
                f"  - Republican vote share baseline: adopters = {adopter['rep_vote_share'].mean():.3f}, non-adopters = {non['rep_vote_share'].mean():.3f}",
                f"  - Gun ownership baseline: adopters = {adopter['gun_ownership'].mean():.3f}, non-adopters = {non['gun_ownership'].mean():.3f}",
                f"  - Rurality: adopters = {adopter['rurality'].mean():.3f}, non-adopters = {non['rurality'].mean():.3f}",
                f"  - Baseline firearm suicide: adopters = {adopter['baseline_firearm_suicide'].mean():.3f}, non-adopters = {non['baseline_firearm_suicide'].mean():.3f}",
                "",
            ]
        )
...
def main():
    did, welch, hetero, state_level = load_tables()

    sections = [
        build_main_conclusion(did, welch),
        outcome_section("Firearm Suicide", did, welch, hetero),
        outcome_section("Non-Firearm Suicide", did, welch, hetero),
        outcome_section("Total Suicide", did, welch, hetero),
        outcome_section("Firearm Homicide", did, welch, hetero),
        outcome_section("Total Firearm Deaths", did, welch, hetero),
        build_heterogeneity_section(hetero),
        build_political_selection_section(state_level),
        build_abstract_style_section(did),
